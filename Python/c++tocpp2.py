...
def cpp_a_pseudocodigo(codigo_cpp):
    texto = codigo_cpp

    # Reemplazos básicos
    texto = re.sub(r'cin\s*>>', 'Leer ', texto)
    texto = re.sub(r'cout\s*<<', 'Escribir ', texto)
    texto = re.sub(r'system\s*\(\s*["\']pause["\']\s*\)\s*;', 'pausar pantalla', texto)
    texto = re.sub(r'system\s*\(\s*["\']cls["\']\s*\)\s*;', 'refrescar pantalla', texto)

    # if, else if, else
    texto = re.sub(r'\bif\s*\((.*?)\)', r'SI (\1) ENTONCES:', texto)
    texto = re.sub(r'\belse if\s*\((.*?)\)', r'SINO SI (\1) ENTONCES:', texto)
    texto = re.sub(r'\belse\b', 'SINO', texto)

    # while
    texto = re.sub(r'\bwhile\s*\((.*?)\)', r'MIENTRAS (\1) HACER:', texto)

    # for mejorado
    def convertir_for(match):
        inicio = match.group(1).strip()
        condicion = match.group(2).strip()
        incremento = match.group(3).strip()

        # Intentar extraer variable y valores
        m_inicio = re.match(r'(int\s+)?(\w+)\s*=\s*(\d+)', inicio)
        if not m_inicio:
            return f"PARA {inicio}; {condicion}; {incremento} HACER:"

        var = m_inicio.group(2)

        m_cond = re.match(rf'{var}\s*[<>=!]+\s*\w+', condicion)
        m_incr = re.match(rf'{var}\+\+|(\+\+{var})|{var}\s*=\s*{var}\s*\+\s*1', incremento)

        if m_cond and m_incr:
            return f"PARA {inicio}; {condicion}; {incremento} HACER:"
        else:
            return f"PARA {inicio}; {condicion}; {incremento} HACER:"

    texto = re.sub(r'\bfor\s*\((.*?);(.*?);(.*?)\)', convertir_for, texto)

    # Reemplazar punto y coma por salto de línea
    texto = texto.replace(';', '\n')
    texto = texto.replace('<<endl', 'endl\n')
    texto = texto.replace('<< endl', 'endl\n')
    texto = texto.replace('"', '')
    texto = texto.replace('endl', 'endl')
...
pseudocodigo = cpp_a_pseudocodigo(codigo_cpp)
print(pseudocodigo)

...
def cpp_to_pseudocode(cpp_code):
    ...
    for line in lines:
        ...
        line = re.sub(r'(int|float|double|char|bool|string)\s+(\w+)', r'Definir \2 como \1', line)

        # Entrada y salida
        line = line.replace('cin>>', 'Leer')
        line = line.replace('cout<<', 'Escribir')

        line = line.replace('system("pause")', 'pausa')
        line = line.replace('system("cls")', 'refresco')


        # Estructuras de control
        line = line.replace('if (', 'Si ')
        line = line.replace('else if (', 'Sino si ')
        line = line.replace('else', 'Sino')
        line = line.replace('for (', 'Para ')
        line = line.replace('while (', 'Mientras ')
...
# Aquí pones el código C++ que quieres convertir
cpp_code = """
#include <iostream>
#include <cstdlib>
#include <ctime>
...
# Convertir y mostrar
print(cpp_to_pseudocode(cpp_code))
